...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://localhost:8080/api/orgs/login", {
        method: "POST",
      ...
      if (res.ok) {
        ...
        // In a real app we'd use cookies/context, for prototype local storage is fine
        localStorage.setItem("ngoUser", JSON.stringify(orgData));
  ...
  return (
    <>
      <Head>
        <title>Responder Login | Shelter Connect</title>
      </Head>
      <div className="max-w-md mx-auto py-12 px-4">
        <div className="bg-white dark:bg-gray-900 p-8 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800">
          <div className="text-center mb-8">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Responder Login</h1>
            <p className="text-gray-500 dark:text-gray-400 mt-2">Access the dashboard to view and claim cases.</p>
          </div>
          ...
          <p className="text-center mt-6 text-sm text-gray-500 dark:text-gray-400">
            Don't have an account? <Link href="/ngo/register" className="text-rose-600 dark:text-rose-400 font-medium hover:underline">Register here</Link>
          </p>
        </div>
      </div>
    </>
